    ...
    async function simulateCallWithRetry() {
      for (let i = 0; i <= MAX_RETRIES; i++) {
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Error after ${i + 1} attempts: Status ${response.status}`,
            );
          }
...
function testFallbackPlan() {
  ...
  const fallbackPlan = {
    ...
    Q4 : Optimize Diabetes MedsChecks`,
      milestones: ["Q1: Training completed", "Q2: Services launched"],
    },
    actions: {
      staffing: [
  ...
  if (fallbackPlan.timeline && fallbackPlan.timeline.timelineChart) {
    console.log("✓ Timeline with Timeline chart present");
  } else {
    console.log("✗ Missing timeline or Timeline chart");
  }

  if (
    fallbackPlan.timeline &&
    fallbackPlan.timeline.milestones &&
    Array.isArray(fallbackPlan.timeline.milestones)
  ) {
    console.log(
      `✓ ${fallbackPlan.timeline.milestones.length} milestones defined`,
    );
  } else {
  ...
  if (fallbackPlan.actions && Object.keys(fallbackPlan.actions).length > 0) {
    console.log(
      `✓ Actions defined for ${Object.keys(fallbackPlan.actions).length} categories`,
    );
  } else {
  ...
  if (
    fallbackPlan.financials &&
    fallbackPlan.financials.investmentRequired &&
    fallbackPlan.financials.projectedROI
  ) {
    console.log(
      `✓ Financials: $${fallbackPlan.financials.investmentRequired} investment, ${fallbackPlan.financials.projectedROI}x ROI`,
    );
  } else {
...
function testErrorMessages() {
  ...
  const errorScenarios = [
    ...
    {
      status: 401,
      expected: "Invalid API key - please check your OpenRouter configuration",
    },
    {
      status: 402,
      expected: "Insufficient credits - your API key has run out of credits",
    },
    {
      status: 429,
      expected:
        "Rate limit exceeded - you have sent too many requests. Please wait a moment and try again.",
    },
    {
      status: 502,
      expected:
        "Bad gateway - temporary issue with the AI provider. Please wait a few minutes and try again.",
    },
    {
      status: 503,
      expected:
        "Service unavailable - the AI service is temporarily down. Please try again later.",
    },
  ];
