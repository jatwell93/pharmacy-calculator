  ...
  part1: [
    {
      ...
      potentialVol: 90,
      calc: standardCalc,
      assumptions: "Assumes one patient visit per week for 52 weeks.",
    },
    {
      id: "daa-private",
      name: "DAA - private",
    ...
    },
    {
      id: "staged-supply",
      name: "Staged Supply",
      fundedRateDesc: "$8.12 first + $4.12 each additional pickup",
      assumptions:
        "Enter how many times per week they pick up on average for up to 15 patients.",
      fundedRate: 8.12, // Changed to first day rate
      patientFeeCurrent: 0,
      patientFeePotential: 0,
      unit: "Patients",
      timeFactor: 52,
      ...
      potentialVol: 15,
      calc: (patientPickups) => {
        const STAGED_SUPPLY_FIRST_DAY = 8.12;
        const STAGED_SUPPLY_SUBSEQUENT_DAY = 4.12;
        ...
        for (let pickups of patientPickups) {
          ...
          if (p > 0) {
            ...
            yearlyRevenue += weeklyFee * WEEKS_PER_YEAR;
          }
        }
        return [yearlyRevenue, 0]; // Only current value, no additional calc here
      },
      customLayout: true,
    },
    {
      ...
      assumptions:
        "The monthly cap is 20 MedsChecks (including Diabetes MedsChecks). Is a 10/10 split between MedsChecks and Diabetes MedsChecks realistic for your pharmacy? If not, adjust the split below.",
    },
    {
      id: "hmr",
      name: "HMRs",
      ...
      customLayout: true,
      assumptions:
        "Enter the number of reviews your pharmacy conducts in an average month. If you pay a third party to conduct reviews on behalf of your pharmacy enter the % of the total payment that is passed onto your third party provider.",
      calc: (volume, thirdPartyPercent) => {
        const grossRevenue = volume * 222.77 * 12; // monthly to yearly
        const netRevenue = grossRevenue * (1 - thirdPartyPercent / 100);
        const thirdPartyCost = grossRevenue - netRevenue;
        return [netRevenue, 0]; // Current net, no additional calc
      },
    },
    {
      id: "hmr-f1",
    ...
    },
    {
      ...
      customLayout: false,
      assumptions:
        "Do you provide and claim RMMR services within an approved Australian Government-funded Aged Care Facility/Facilities?",
      calc: standardCalc,
    },
    {
      id: "rmmr-f1",
    ...
    },
    {
      ...
      customLayout: false,
      assumptions:
        "Do you provide and claim QUM services within an approved Australian Government-funded Aged Care Facility/Facilities? Enter the number of eligible aged care beds within the Facilities serviced by you. If there is no opportunity to service a Facility, enter 0.",
      potentialVol: 75,
  ...
  part4: [
    {
      ...
      calc: () => {
        ...
        const programGap = biosimilar - program;
        const programOpportunity = Math.max(0, programGap) * ADA_PROGRAM;
        const totalOpportunity = marginOpportunity + programOpportunity;
        return [0, totalOpportunity];
      },
    },
    {
      id: "etanercept",
      ...
      calc: () => {
        ...
        const programGap = biosimilar - program;
        const programOpportunity = Math.max(0, programGap) * ETA_PROGRAM;
        const totalOpportunity = marginOpportunity + programOpportunity;
        return [0, totalOpportunity];
      },
    },
